  ...
  useEffect(() => {
    ...
    class FluidParticle {
      ...
      update() {
        ...
        gradient.addColorStop(0.4, `hsla(${this.hue + 20}, ${this.saturation}%, ${this.lightness + 10}%, ${alpha * 0.3})`); // Further reduced
        gradient.addColorStop(1, `hsla(${this.hue + 40}, ${this.saturation}%, ${this.lightness + 20}%, 0)`);
    ...
    const createParticles = (x, y, prevX, prevY) => {
      ...
      const distance = Math.sqrt(dx * dx + dy * dy);
        ...
        for (let i = 0; i < steps; i++) {
          ...
          for (let j = 0; j < 2; j++) { // Reduced from 3 to 2 particles per step
            const angle = Math.random() * Math.PI * 2;
            const speed = Math.random() * 2 + 0.5;
            const vx = Math.cos(angle) * speed + dx * 0.1;
            const vy = Math.sin(angle) * speed + dy * 0.1;
            
            particles.current.push(new FluidParticle(
      ...
      if (particles.current.length > 400) { // Reduced from 800 to 400
