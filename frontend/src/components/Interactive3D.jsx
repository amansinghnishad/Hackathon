...
const Interactive3D = () => {
  ...
  const initialElements = [
    { id: 1, type: 'cube', color: '#ff6b6b', size: 'large', 
      position: { x: -200, y: -100 }, velocity: { x: 0, y: 0 }, basePosition: { x: -200, y: -100 } },
    { id: 2, type: 'sphere', color: '#4ecdc4', size: 'medium', 
      position: { x: 100, y: -150 }, velocity: { x: 0, y: 0 }, basePosition: { x: 100, y: -150 } },
    { id: 3, type: 'pyramid', color: '#45b7d1', size: 'small', 
      position: { x: -50, y: 50 }, velocity: { x: 0, y: 0 }, basePosition: { x: -50, y: 50 } },
    { id: 4, type: 'cylinder', color: '#f9ca24', size: 'medium', 
      position: { x: 200, y: 100 }, velocity: { x: 0, y: 0 }, basePosition: { x: 200, y: 100 } },
    { id: 5, type: 'octahedron', color: '#6c5ce7', size: 'large', 
      position: { x: -100, y: 150 }, velocity: { x: 0, y: 0 }, basePosition: { x: -100, y: 150 } },
    { id: 6, type: 'torus', color: '#fd79a8', size: 'small', 
      position: { x: 50, y: -50 }, velocity: { x: 0, y: 0 }, basePosition: { x: 50, y: -50 } },
    { id: 7, type: 'cube', color: '#00b894', size: 'medium', 
      position: { x: 150, y: -25 }, velocity: { x: 0, y: 0 }, basePosition: { x: 150, y: -25 } },
    { id: 8, type: 'sphere', color: '#e17055', size: 'small', 
      position: { x: -150, y: 0 }, velocity: { x: 0, y: 0 }, basePosition: { x: -150, y: 0 } },
    { id: 9, type: 'diamond', color: '#a29bfe', size: 'large', 
      position: { x: 0, y: 100 }, velocity: { x: 0, y: 0 }, basePosition: { x: 0, y: 100 } },
    { id: 10, type: 'hexagon', color: '#fd63a1', size: 'medium', 
      position: { x: 75, y: 75 }, velocity: { x: 0, y: 0 }, basePosition: { x: 75, y: 75 } }
  ];
  ...
  const updatePhysics = useCallback(() => {
    ...
    setElements(prevElements => 
      prevElements.map(element => {
        ...
        const springX = (element.basePosition.x - element.position.x) * springForce;
        const springY = (element.basePosition.y - element.position.y) * springForce;
  ...
  const getTransform = (element) => {
    ...
    if (distance < visualRadius && distance > 0) {
      ...
      const rotationZ = Math.sin(Date.now() * 0.005 + element.id) * influence * 15;
  ...
  const getElementStyle = (element) => {
    ...
    return {
      ...
      boxShadow: isActive 
        ? `0 ${shadowBlur + 10}px ${shadowSpread + 20}px ${element.color}40, 0 0 ${shadowSpread + 20}px ${element.color}60`
        : `0 ${shadowBlur}px ${shadowSpread + 10}px ${element.color}20, 0 0 ${shadowSpread}px ${element.color}30`,
      zIndex: isActive ? 100 : Math.floor(shadowIntensity * 50) + 1,
      filter: `brightness(${1 + shadowIntensity * 0.3}) saturate(${1 + shadowIntensity * 0.2})`
    };
  };
    ...
    <div className="interactive-3d-section">
      ...
      >        <div className="elements-wrapper">
          {elements.map((element) => (
            <div
              key={element.id}
              className={`element-3d ${element.type} ${element.size} ${activeElement === element.id ? 'active' : ''}`}
        ...
        <div className="interaction-guide">
          <div className="guide-text">
            {activeElement 
              ? `Interacting with ${elements.find(el => el.id === activeElement)?.type} #${activeElement}`
              : isHovering 
